...
# ***** END LICENSE BLOCK *****
#
# This version was forked from /toolkit/crashreporter/tools/ to fix issues
# with space handling.
#
...
class Dumper:
    ...
    def ProcessFile(self, file):
        ...
        # Note that the file name may contain spaces.
        module_regex = re.compile(r'^MODULE\s+\S+\s+\S+\s+(\S+)\s+(.*)$')
        for arch in self.archs:
            try:
                cmd = os.popen("%s %s \"%s\"" % (self.dump_syms, arch, file), "r")
                module_line = cmd.next()
                match = module_regex.match(module_line)
                if match:
                    guid = match.group(1)
                    debug_file = match.group(2)
